...
def check_cardinality(data, threshold=0.005, frac=0.5):
    ...
    for column_stats in cat_features_stats:
        if (column_stats[1].shape[0] == 1) or (
            column_stats[1].shape[0] >= (data.shape[0] * 0.98)
        ):
...
def find_deterministic_columns_binary(df, binary):
    ...
    for j, i in enumerate(binary):
        ...
        score = clf.score(
            ...
            df[i].fillna(0).iloc[jts].astype(np.int32),
        )
        if score > 0.999:
...
def identify_dates(data):
    ...
    for i in data2.columns:
        if str(data2[i].dtype) in ["int", "int32", "int64"]:
            cond1 = (
                ((data2[i] > -1).astype(int) + (data2[i] < 25).astype(int)) == 2
            ).all()
            cond2 = data2[i].nunique() < 25
            cond3 = (
                ((data2[i] > 0).astype(int) + (data2[i] < 12).astype(int)) == 2
            ).all()
            cond4 = data2[i].nunique() < 13
            cond5 = (
                (
                    (data2[i] > 2000).astype(int)
                    + (data2[i] <= datetime.now().year).astype(int)
                )
                == 2
            ).all()
            ...
            if (
                (cond1 and cond2 and cond7)
                or (cond3 and cond4 and cond7)
                or (cond5 and cond6)
            ):
