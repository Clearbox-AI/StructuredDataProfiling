...
class DatasetProfiler:
    ...
    def profile(self, tol: float = 1e-6):
        ...
        unique, binary, id_column, high_cardinality, rare_labels = check_cardinality(
        ...
        self.bivariate_tests, self.anomalies = get_label_correlation(
            xp,
        ...
        num_cols = [
            ...
            if self.column_profiles[i]["dtype"] != object
        ]
        cat_cols = [
            i
            for i in self.column_profiles
        ...
        self.ordinal_columns = find_ordinal_columns(
            self.reduced_data_sample,
            cat_columns,
        )
        ...
        column_search = np.intersect1d(
            list(
                self.reduced_data_sample.columns[
                    self.reduced_data_sample.dtypes != "object"
                ],
            ),
            list(binary.keys()),
        ...
        self.deterministic_columns_binary, num_cols = find_deterministic_columns_binary(
            self.reduced_data_sample,
            column_search,
        )
        ...
        (
            self.deterministic_columns_regression,
            num_cols,
    ...
    def feat_selection(self, frac=0.1):
        ...
        num_cols = [
            ...
            if self.reduced_data_sample[i].dtype != object
        ]
        cat_cols = [
            i
            for i in self.reduced_data_sample.columns
        ...
        temp_df = pd.concat(
            [
                self.reduced_data_sample[num_cols],
                self.reduced_data_sample[cat_cols]
                .astype("category")
        ...
        if self.regression is True:
            feat_select = SelectKBest(mutual_info_regression, k=K).fit(
                temp_df.drop(self.target, axis=1),
                temp_df[self.target],
            )
        else:
            feat_select = SelectKBest(mutual_info_classif, k=K).fit(
                temp_df.drop(self.target, axis=1),
                temp_df[self.target],
            )
        ...
        if (self.sequence_index is not None) and (
            self.sequence_index not in self.feature_selection
        ):
